...
#'
#' @param chromothripsis_output the output of the function detect_chromothripsis
...
#'
#' @export
plot_chromothripsis <- function(chromothripsis_output, chr = chr, BAF = NULL, sample_name = "",
    ...
    if (!(as.character(chr) %in% chromNames)) {
        stop("Chromosome not valid")
    }

    common_ggplot2 <- theme_bw() + theme(
    ...
    summary <- chromothripsis_output@chromSummary
    candidate_chrs <- chromothripsis_output@chromSummary$chrom
    cluster_sizes <- sapply(chromothripsis_output@detail$connComp, length)
    # get the SVs for the cluster of SVs in this chromosome
    cand_clust_size <- chromothripsis_output@chromSummary$clusterSize[chromothripsis_output@chromSummary$chrom == cand]
    idx <- which(cluster_sizes == cand_clust_size)
    SVsnow <- chromothripsis_output@detail$SV ## [ as.numeric(unlist(chromothripsis_output@detail$connComp[idx])) ,]
    CNVsnow <- chromothripsis_output@detail$CNV
    ...
    #-------------------------------------------------------------------------------------------------------
    inter <- chromothripsis_output@detail$SVinter
    ...
    if (nrow(now) > 0) {
        for (i in 1:nrow(now)) {
            SV_plot <- SV_plot + geom_curve(
                size = arc_size, data = now[i, ],
    ...
    SV_plot <- SV_plot + scale_colour_manual(
        name = "SV type",
    ...
    if (max(CNVsnow$total_cn) <= 10) {
        CNV_plot <- ggplot() +
            ...
            scale_x_continuous(expand = c(0.01, 0.01), labels = function(x) {
                paste(x / 1000000, "MB")
    ...
    if (max(CNVsnow$total_cn) <= 100 & max(CNVsnow$total_cn) > 10) {
        idx <- which(CNVsnow$total_cn == 0)
        if (length(idx) > 0) {
            CNVsnow$total_cn[which(CNVsnow$total_cn == 0)] <- 0.99
        }
        ...
        CNV_plot <- ggplot() +
            geom_segment(data = CNVsnow, aes(x = start, y = log(total_cn, base = 2), xend = end, yend = log(total_cn, base = 2)), size = 2) +
            common_ggplot2 +
            ylab("CN") +
            xlab(NULL) +
            # scale_x_continuous(expand = c(0.01,0.01),labels = function(x){paste(x/1000000,"MB")})#,limits=c(min_coord,max_coord))
        ...
        CNV_plot <- CNV_plot + scale_y_continuous(
            minor_breaks = NULL, breaks = log(mm, base = 2),
            limits = c(
                -.01 + log(min(CNVsnow$total_cn), base = 2),
    ...
    if (max(CNVsnow$total_cn) > 100) {
        ...
        CNV_plot <- CNV_plot + scale_y_continuous(
            minor_breaks = NULL, breaks = log(mm, base = 10),
            limits = c(
                log(min(CNVsnow$total_cn), base = 10),
    ...
    ideogram <- ggplot(data = chr_info, aes(x = y, y = y)) +
        geom_point(colour = "white")
    ideogram <- ideogram +
        geom_rect(
            data = chr_info, mapping = aes(xmin = start, xmax = end, ymin = 0, ymax = 1),
            fill = chr_info$color, color = "black", size = .1
        )

    ideogram <- ideogram + ylim(0, 4)
    ideogram <- ideogram + annotate(
        geom = "text", x = (chr_info_annot$start + chr_info_annot$end) / 2, y = chr_info_annot$y + 1.2,
    ...
    summary$oscillations <- paste(summary$max_number_oscillating_CN_segments_2_states, ";",
        summary$max_number_oscillating_CN_segments_3_states,
        sep = ""
    )
    ...
    mytheme <- gridExtra::ttheme_minimal(
        padding = unit(c(1.8, 1.8), "mm"),
        core = list(fg_params = list(cex = .6)),
        colhead = list(fg_params = list(cex = .6)),
        rowhead = list(fg_params = list(cex = .6))
    )

    for (cc in 1:ncol(table_now)) {
        if (is.numeric(table_now[, cc])) {
            table_now[, cc] <- round(table_now[, cc], digits = 2)
        }
    }
